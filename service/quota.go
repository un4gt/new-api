...
func checkAndSendQuotaNotify(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int) {
	gopool.Go(func() {
		...
		if quotaTooLow {
			prompt := "您的额度即将用尽"
			redemptionLink := fmt.Sprintf("%s/console/topup", system_setting.ServerAddress)
			...
			if notifyType == dto.NotifyTypeBark {
				...
				content = "{{value}}，剩余额度：{{value}}，请联系管理员获取兑换码"
				values = []interface{}{prompt, logger.FormatQuota(relayInfo.UserQuota)}
			} else if notifyType == dto.NotifyTypeGotify {
				content = "{{value}}，当前剩余额度为 {{value}}，请联系管理员获取兑换码。"
				values = []interface{}{prompt, logger.FormatQuota(relayInfo.UserQuota)}
			} else {
				// 默认内容格式，适用于Email和Webhook（支持HTML）
				content = "{{value}}，当前剩余额度为 {{value}}，请联系管理员获取兑换码。<br/>兑换码页面：<a href='{{value}}'>{{value}}</a>"
				values = []interface{}{prompt, logger.FormatQuota(relayInfo.UserQuota), redemptionLink, redemptionLink}
			}
...
func checkAndSendSubscriptionQuotaNotify(relayInfo *relaycommon.RelayInfo) {
	gopool.Go(func() {
		...
		if notifyType == dto.NotifyTypeBark {
			...
			content = "{{value}}，当前剩余额度为 {{value}}，请联系管理员获取兑换码。<br/>兑换码页面：<a href='{{value}}'>{{value}}</a>"
			values = []interface{}{prompt, logger.FormatQuota(int(remaining)), redemptionLink, redemptionLink}
		}
