...
export function ChannelMutateDrawer({
  ...
  const [isCodexCredentialRefreshing, setIsCodexCredentialRefreshing] =
    useState(false)
  const [isTestingBalanceScript, setIsTestingBalanceScript] = useState(false)
  ...
  const currentCustomBalanceScript = form.watch('custom_balance_script')
  ...
  const extraSettingsConfigured = Boolean(
    ...
    currentCustomBalanceScript?.trim() ||
    currentProxy?.trim() ||
  ...
  const handleTestBalanceScript = useCallback(async () => {
    if (!channelId) return

    const script = String(form.getValues('custom_balance_script') || '').trim()
    if (!script) {
      toast.info(t('Please enter a custom balance query script'))
      return
    }

    setIsTestingBalanceScript(true)
    try {
      const res = await testChannelBalanceScript(channelId, script)
      if (!res.success) {
        throw new Error(res.message || t('Failed to test balance script'))
      }
      const balanceText =
        typeof res.balance === 'number' ? `: ${res.balance}` : ''
      toast.success(`${t('Balance script test succeeded')}${balanceText}`)
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : t('Failed to test balance script')
      )
    } finally {
      setIsTestingBalanceScript(false)
    }
  }, [channelId, form, t])
                            ...
                            {currentType === 18 && (
                              <FormField
                                ...
                                render={({ field }) => (
                                  <FormItem>
                                    <FormLabel>
                                      {t('Service ID or Extra Configuration *')}
                                    </FormLabel>
                                    <FormControl>
                                      <Input
                                        placeholder={t(
                                          'Xunfei Xingchen service identifier'
                                        )}
                                        {...field}
                                      />
                                    </FormControl>
                                    <FormDescription>
                                      {t(
                                        'Required by Xunfei Xingchen embedding and rerank services.'
                                      )}
                                    </FormDescription>
                                    <FormMessage />
                                  </FormItem>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            <FormField
                              control={form.control}
                              name='custom_balance_script'
                              render={({ field }) => (
                                <FormItem>
                                  <div className='flex items-center justify-between gap-2'>
                                    <FormLabel>
                                      {t('Custom Balance Query Script')}
                                    </FormLabel>
                                    {isEditing && (
                                      <Button
                                        type='button'
                                  ...
                                  <FormControl>
                                    <Textarea
                                      placeholder={t(
                                        'Lua script used to query and return channel balance'
                                      )}
                                      rows={6}
                                  ...
                                  <FormDescription>
                                    {t(
                                      'Optional Lua script for providers whose balance API is not natively supported.'
                                    )}
                                  </FormDescription>
                                  <FormMessage />
                                </FormItem>
