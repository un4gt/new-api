...
// Get current session registration invite activation state
export async function getRegistrationInviteActivation(): Promise<
  ApiResponse & { data?: RegistrationInviteActivation }
> {
  const res = await api.get('/api/registration-invite/activation')
  return res.data
}

// Activate a registration invite code for the current session
export async function activateRegistrationInvite(
  code: string
): Promise<ApiResponse & { data?: RegistrationInviteActivation }> {
  const res = await api.post('/api/registration-invite/activate', { code })
  return res.data
}
