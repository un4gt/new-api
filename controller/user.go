...
func Logout(c *gin.Context) {
	...
	preserveActivation := activation != nil && activation.ExpiresAt > common.GetTimestamp()
	session.Clear()
	if preserveActivation {
		setRegistrationInviteActivation(session, activation)
	}
	err := session.Save()
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
...
func Register(c *gin.Context) {
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if activation != nil {
			return model.ConsumeRegistrationInviteTx(tx, activation.InviteID, cleanUser.Id, "password", cleanUser.Username)
		}
		return nil
	})
	if err != nil {
	...
	cleanUser.FinalizeOAuthUserCreation()
	if activation != nil {
...
func GetAllUsers(c *gin.Context) {
	...
	sanitizeUsersForResponse(users)
...
func sanitizeUserForResponse(user *model.User) {
	if user == nil {
		return
	}
	user.AffCode = ""
	user.AffCount = 0
	user.AffQuota = 0
	user.AffHistoryQuota = 0
	user.InviterId = 0
}

func sanitizeUsersForResponse(users []*model.User) {
...
func UpdateSelf(c *gin.Context) {
	...
	// Minimal build: remove personal preferences updates (e.g. language/sidebar modules).
	// The UI keeps check-in and account/security settings only.
	if _, ok := requestData["sidebar_modules"]; ok {
		common.ApiErrorMsg(c, "不支持的设置项")
		return
	}
	if _, ok := requestData["language"]; ok {
		common.ApiErrorMsg(c, "不支持的设置项")
		return
	}
...
func TopUp(c *gin.Context) {
	...
	if !lock.TryLock() {
		common.ApiErrorMsg(c, "兑换处理中，请稍后重试")
		return
	}
	defer lock.Unlock()
	req := topUpRequest{}
...
func writeRedeemError(c *gin.Context, err error) {
	...
	case errors.Is(err, model.ErrRedeemFailed):
		common.ApiErrorI18n(c, i18n.MsgRedemptionFailed)
	default:
		common.ApiError(c, err)
	}
}
