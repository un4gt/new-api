...
// GetChannelKey 获取渠道密钥（一般需要通过安全验证中间件）
// 依赖 SecureVerificationRequired 中间件；Root 用户可跳过安全验证。
...
func validateChannel(channel *model.Channel, isAdd bool) error {
	// Guard against nil channel payload to avoid panic on malformed requests.
	if channel == nil {
...
func FetchModels(c *gin.Context) {
	...
	if req.Type == constant.ChannelTypeNvidia {
		c.JSON(http.StatusOK, gin.H{
			"success": true,
			"data":    nvidia.ModelList,
		})
		return
	}
